    ...
    func registrationApi () {
        ...
        ApiClient.shared.getData("GET", url, post, getList.self) { [self] (sucess, resp, msg) in
                    if sucess{
                        ...
                        print(response.status?.type!)
                        print(resp)
                        print(response.data![0].details?.memberFee![1].amount)
                        ...
                        let y = response.data![0].details?.memberFee?.count
                        if y == 0 { } else {
                            for index in 0...y!-1
                            {
                                memberFeeArray.append((response.data![0].details?.memberFee![index].amount)!)
                            }
                        }
